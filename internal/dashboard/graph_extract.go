...
var (
	...
	graphTechPattern    = regexp.MustCompile(
	...
	graphNumberedTopicPattern = regexp.MustCompile(`^(?:[0-9]+(?:\.[0-9]+)*[.)]?|[IVXivx]+[.)])\s+`)
	graphTypeFragmentPattern  = regexp.MustCompile(`[{}\[\],;:=<>]`)
	graphNoisyCodePattern     = regexp.MustCompile(`(?i)(?:error|exception|tests?|testcase)$`)
)
...
var graphGenericTerms = stringSet(
	...
	"not_configured", "installed", "接口", "服务", "模型", "算法", "模块", "数据库",
)

var graphLowInformationTerms = stringSet(
	"if", "in", "node", "end", "exists", "claim", "textcontent", "text not null",
	"argumentparser", "pureposixpath", "scanresult", "scanstate", "zipfile", "uuid",
	"za", "it", "this", "every", "local", "software", "systemexit", "ids", "join-path",
...
var graphEntityPatterns = []graphPattern{
	{expression: graphWikiPattern, kind: "concept", confidence: 0.98, capture: 1, source: "wiki"},
	{expression: graphCodePattern, kind: "term", confidence: 0.90, capture: 1, source: "code"},
	{expression: graphTechPattern, kind: "concept", confidence: 0.78, source: "tech"},
	{expression: graphEnglishPattern, kind: "term", confidence: 0.72, source: "english"},
}
...
func canonicalGraphTopic(value string) string {
	return graphNumberedTopicPattern.ReplaceAllString(canonicalGraphLabel(value), "")
}
...
func graphLineSpans(line string, lineNumber int) []graphSpan {
	...
	if match := graphHeadingPattern.FindStringSubmatchIndex(line); len(match) >= 4 {
		label := canonicalGraphTopic(line[match[2]:match[3]])
		if validGraphLabel(label, "heading") {
	...
	for _, pattern := range graphEntityPatterns {
		for _, match := range pattern.expression.FindAllStringSubmatchIndex(line, -1) {
			...
			if !validGraphLabel(label, pattern.source) {
				continue
			}
			rows = append(rows, graphSpan{
...
func validGraphLabel(label, source string) bool {
	label = norm.NFKC.String(label)
	folded := strings.ToLower(label)
	length := utf8.RuneCountInString(label)
	if length < 2 || length > 80 {
		return false
	}
	if _, generic := graphGenericTerms[folded]; generic {
		return false
	}
	if _, lowInformation := graphLowInformationTerms[folded]; lowInformation {
		return false
	}
	if graphTypeFragmentPattern.MatchString(label) {
		return false
	}
	if (source == "code" || source == "english") && graphNoisyCodePattern.MatchString(label) {
		return false
	}
	if source == "tech" {
		chinese := 0
...
func stringSet(values ...string) map[string]struct{} {
	...
	for _, value := range values {
		result[strings.ToLower(norm.NFKC.String(value))] = struct{}{}
	}
	return result
}
