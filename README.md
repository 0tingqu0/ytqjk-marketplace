...
Create a new task after installation; existing tasks do not reload bundled skills. The current release version is pure SemVer `0.6.10`. `+codex.*` is only for refreshing local development caches and is neither committed nor included in release manifests.
