...
Create a new task after installation; existing tasks do not reload bundled skills. The current release version is `0.6.2.1`: release tags and the dashboard support three or four numeric segments, with the optional fourth segment representing a small revision. Codex plugin manifests remain strict three-segment SemVer (`0.6.3` for this release). `+codex.*` is only for refreshing local development caches and is neither committed nor included in release manifests.
