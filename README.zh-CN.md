...
SemVer `0.6.10`；`+codex.*` 仅供本地开发临时缓存刷新，不提交、不进入正式发布清单。
...
并对经授权的远端根库及其本地子树执行显式检索和材料读取。网页管理端始终只监听本机；局域网
...
可选范围包括项目标识内的本机库，以及与该项目同级的非挂载本机库。可选的 `remote_node_id`
是本机已选择访问的远端根库。因此，被访问端可以先保存仅允许入站的
授权；访问端再通过鉴权获取对方实际开放的 `export_nodes`，选择其中一个后才启用出站查询。
旧版单根 `export_node_id` 配置会在读取时迁移。跨电脑查询只能读取被授权根库及其本地后代，
绝不向上访问其父库，也不读取未开放的兄弟库或经挂载节点转发到第三台电脑。
