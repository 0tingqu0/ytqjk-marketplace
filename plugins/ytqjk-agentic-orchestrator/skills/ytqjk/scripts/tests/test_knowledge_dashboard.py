...
class KnowledgeDashboardTest(unittest.TestCase):
    ...
    def test_project_library_read_does_not_create_empty_cache(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertFalse((project / "cache" / "global-knowledge.sqlite3").exists())
    ...
    def test_project_library_lists_prefetched_global_knowledge(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            cache.write_text(
                '{"entries":[{"path":"personal-experience/approved/session.md",'
                '"line_start":1,"line_end":2,"content":"cached knowledge",'
                '"query":"cached"}]}',
                encoding="utf-8",
            )
