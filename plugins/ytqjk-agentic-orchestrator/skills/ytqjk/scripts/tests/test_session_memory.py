...
def register_projects(root: Path, *project_ids: str) -> None:
    projects = {}
...
class SessionMemoryTest(unittest.TestCase):
    def test_anchor_checkpoint_restore_and_confirmed_archive(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            register_projects(root, "project-a")
            MODULE.write_anchor(root, "thread-1", "project-a")
            MODULE.checkpoint(
                root,
                "thread-1",
                "project-a",
                "结论：测试通过；证据：commit abc。",
            )
            ...
            candidates = root / "personal-experience/candidates"
            self.assertEqual(len(list(candidates.glob("*.md"))), 1)

    def test_sweep_archives_only_inactive_anchored_memory(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            register_projects(root, "project-a")
            anchor = MODULE.checkpoint(
                root,
                "thread-2",
                "project-a",
                "来源：测试记录。" * 20,
            )
            inactive = datetime.now(timezone.utc) - timedelta(days=31)
            anchor["last_activity_at"] = inactive.isoformat()
            MODULE.anchor_path(root, "thread-2").write_text(
    ...
    def test_corrupt_anchor_is_not_overwritten(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            register_projects(root, "project-b")
    ...
    def test_repeated_anchor_uses_one_stable_anchor_path(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            register_projects(root, "project-a")
            _, created_first = MODULE.ensure_anchor(
                root, "thread-5", "project-a"
            )
            _, created_second = MODULE.ensure_anchor(
                root, "thread-5", "project-a"
            )

            self.assertTrue(created_first)
            self.assertFalse(created_second)
            anchors = list((root / "sessions").glob("*/anchor.json"))
            self.assertEqual(len(anchors), 1)

    def test_inspect_distinguishes_session_states(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            register_projects(root, "project-a")
    ...
    def test_anchor_rejects_cross_project_access(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            register_projects(root, "project-a", "project-b")
