---
name: caveman
description: Ultra-compressed communication mode. Cuts token usage by dropping filler, articles, and pleasantries while keeping full technical accuracy. Use when the user asks for caveman mode, terse output, fewer tokens, or brief technical responses.
---

# Caveman

Derived from Matt Pocock's `caveman` skill under the MIT License. See
[LICENSE](LICENSE). The plugin-level source and change notice is in
`THIRD_PARTY_NOTICES.md` when installed as a plugin.
...
> **Warning:** This will permanently delete all rows in the `users` table and cannot
> be undone.
>
> ```sql
> DROP TABLE users;
